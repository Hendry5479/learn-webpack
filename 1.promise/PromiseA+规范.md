...
7. resolvePromise
   ...
   ```
    ...
    7.1 如果 promise2 == x ，reject TypeError
    7.2 如果 x 是 promsie
            如果x是pending，promise必须pending
            如果 x 被 fulfilled, fulfill promise
            如果 x 被 rejected, reject promise 
    7.3 如果 x 是object 或者 function
        let then = x.then.
        如果 x.then 这步出错，那么reject promise
        如果 then 是函数，then.call(x, resolvePromiseFn, rejectPromise)
